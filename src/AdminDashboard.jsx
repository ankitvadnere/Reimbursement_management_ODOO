...
function UsersTab() {
  ...
  const handleAddUser = () => {
    addUser({ name: "New User", email: `user${Date.now()}@acme.com`, password: "pass123", role: "employee", manager_id: null });
  };
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full sticky-table">
            ...
            <tbody>
              {users.map((user, i) => (
                <UserRow key={user.id} user={user} managers={managers} onUpdate={updateUser} isAlt={i % 2 === 1} />
              ))}
            </tbody>
          </table>
        </div>
...
function UserRow({ user, managers, onUpdate, isAlt }) {
  const [name, setName] = useState(user.name);
  const [email, setEmail] = useState(user.email);
  const [editing, setEditing] = useState(!user.email.includes("@acme.com") && user.name === "New User");

  const save = () => { onUpdate(user.id, { name, email }); setEditing(false); };

  return (
    <tr className={`border-b border-gray-100 ${isAlt ? "bg-gray-50/50" : "bg-white"}`}>
      ...
      <td className="px-4 py-3">
        <select value={user.role} onChange={e => onUpdate(user.id, { role: e.target.value })}
      ...
      <td className="px-4 py-3">
        <select value={user.manager_id || ""} onChange={e => onUpdate(user.id, { manager_id: e.target.value ? parseInt(e.target.value) : null })}
          className="border border-gray-300 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-[#1E3A5F]">
      ...
      <td className="px-4 py-3">
        <div className="flex gap-2">
          {editing ? (
            <>
              <button onClick={save} className="bg-[#1E3A5F] text-white px-3 py-1 rounded text-xs hover:bg-[#2d5a8e]">Save</button>
              <button onClick={() => setEditing(false)} className="border border-gray-300 text-gray-600 px-3 py-1 rounded text-xs hover:bg-gray-50">Cancel</button>
            </>
          ) : (
            <>
              <button onClick={() => setEditing(true)} className="border border-gray-300 text-gray-600 px-3 py-1 rounded text-xs hover:bg-gray-50">Edit</button>
              <button onClick={() => alert(`Password sent to ${user.email}`)}
...
function ApprovalRulesTab() {
  const { users, approvalRules, addApprovalRule, showToast } = useApp();
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm text-gray-700">Enforce sequence</span>
            </label>
          ...
          {approvalRules.length === 0 ? (
            ...
            <div className="space-y-3">
              {approvalRules.map(rule => {
                ...
                const mgr = allUsers.find(u => u.id === parseInt(rule.manager_id));
                return (
                  <div key={rule.id} className="border border-gray-200 rounded-lg p-3">
                    ...
                    <div className="text-xs text-gray-500 mt-1 space-y-0.5">
                      ...
                      <p>Manager: <span className="text-gray-700">{mgr?.name || "—"}</span></p>
                      <p>Condition: <span className="text-gray-700">{rule.condition_type}</span></p>
                      {rule.enforce_sequence && <p className="text-[#1E3A5F]">✓ Enforce sequence</p>}
                    </div>
                  </div>
                );
              })}
